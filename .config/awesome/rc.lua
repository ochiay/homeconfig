...
local function go_to(cmd_class, cmd_name, tag_number)
   awful.tag.viewonly(tag_names[tag_number])
   run_or_raise(cmd_class, cmd_name)
end
...
awful.screen.connect_for_each_screen( function(s)
      ...
      -- Create the wibox
      -- s.mywibox = awful.wibar({ position = 'top', screen = s, visible = true, height=34 })
...
globalkeys = awful.util.table.join(
   ...
   awful.key({ modkey,           }, ';',
      ...
      {description = 'popup a ranger', group = 'launcher'}),

   -- launcher
   awful.key({ modkey,           }, 'e',
      function ()
         run_or_raise('Emacs', 'emacsclient -c')
      end,
   ...
   awful.key({ modkey,           }, ',',
      function ()
         view_tag(8)
         run_or_raise(file_manager:sub(1,1):upper()..file_manager:sub(2), file_manager)
         --file_manager:toggle()
      end,
...
clientkeys = awful.util.table.join(
   ...
   awful.key({ modkey, altkey }, 'Prior',
      function (c) c:relative_move(-20, -20,  40,  40) end),
   awful.key({ modkey, }, 'Up',
      function (c)
