...
(with-eval-after-load 'package
  ...
  ;;(add-to-list 'package-archives '("reserv melpa" . "https://www.mirrorservice.org/sites/melpa.org/packages/")))
...
(use-package pyvenv
  :config
  (pyvenv-mode 1)
  ;; чтобы eglot видел активный venv
  (add-hook 'pyvenv-post-activate-hooks
            (lambda ()
              (setq eglot-workspace-configuration
                    `(:python
                      (:venvPath ,pyvenv-virtual-env
                       :pythonPath ,(concat pyvenv-virtual-env "/bin/python"))))
              (eglot-reconnect))))
...
(use-package eglot
  :defer t
  :hook ((python-mode django-mode web-mode rust-ts-mode dart-mode) . eglot-ensure)
  :config
  ...
  ;; (add-to-list 'eglot-server-programs
  ;;              '(dart-mode . ("/snap/bin/dart" "language-server" "--protocol=lsp")))
)
