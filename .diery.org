...
* GIT
-
- git update-index --no-assume-unchanged .emacs.d/init.el
...
django.contrib.auth.models : User, Group, Permission

** Three ways to override default user model:

- django.contrib.auth.models import AbstactUserModel
- django.contrib.auth.models import AbstractBaseUserModel
- OneToOne field to UserModel
...
admin.site.register(model, admin class)
...
+ request ::
  ...
  GET - словарь с гет-параметрами(адресная строка)
...
apfirst -- uppercase
** forms
cleaned_data -- after calling method "is_valid" forms fill this parameter
*** methods
clean
*** create form class
**** ModelForm
...
| Command                | Description                                     |
|------------------------+-------------------------------------------------|
| ps                     | Show a table of all the running containers      |
| images                 | Show a table of all the local images            |
| inspect <container id> | See more information about a container          |
| pull <image id>        | Similar to git pull, it pulls image updates     |
| logs <container id>    | See logs from a container, useful for debugging |
| rm <container id>      | Removes a running container                     |
| rmi <image id>         | Removes an image                                |
| stop <container id>    | Stop a running container                        |
...
# Install Docker based on the manager
case "$PKG_MANAGER" in
    apt)
...
adduser, usermod, chmod, chown, newgrp

** transparency

mkdir .devilspie
touch .devilspie/opacity.ds
echo '( if
...
exec_always "setxkbmap -model pc104 -layout us,ru -variant , ctrl:swapcaps, -option grp:ctrls_toggle"
